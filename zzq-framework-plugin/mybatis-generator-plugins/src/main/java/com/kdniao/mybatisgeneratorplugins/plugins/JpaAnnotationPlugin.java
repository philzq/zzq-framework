package com.zzq.mybatisgeneratorplugins.plugins;
...
public class JpaAnnotationPlugin extends PluginAdapter {
    ...
    public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        String aliasTableName = introspectedTable.getFullyQualifiedTable().getAlias();
        ...
        //如果定义别名，则使用这个别名作为tableName来应对分表场景
        if(aliasTableName != null && aliasTableName.length() > 0){
            topLevelClass.addAnnotation(String.format("@Table(name = \"%s\")", aliasTableName));
        }else{
